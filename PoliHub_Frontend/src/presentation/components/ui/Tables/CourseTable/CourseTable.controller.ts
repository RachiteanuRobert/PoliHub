 ...
 */
export const useCourseTableController = (search: string) => {
    const { getCourses: { key: queryKey, query }, deleteCourse: { key: deleteCourseKey, mutation: deleteCourse } } = useCourseApi();
    ...
    const { mutateAsync: deleteMutation } = useMutation([deleteCourseKey], deleteCourse); // Use a mutation to remove an entry.
