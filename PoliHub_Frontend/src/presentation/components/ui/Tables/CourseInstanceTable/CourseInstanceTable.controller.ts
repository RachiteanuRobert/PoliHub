 ...
 */
export const useCourseInstanceTableController = (search: string) => {
    const { getCourseInstances: { key: queryKey, query }, deleteCourseInstance: { key: deleteCourseInstanceKey, mutation: deleteCourseInstance } } = useCourseInstanceApi();
    ...
    const { mutateAsync: deleteMutation } = useMutation([deleteCourseInstanceKey], deleteCourseInstance); // Use a mutation to remove an entry.
