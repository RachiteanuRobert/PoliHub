 ...
 */
export const useLaboratoryTableController = (search: string) => {
    const { getLaboratories: { key: queryKey, query }, deleteLaboratory: { key: deleteLaboratoryKey, mutation: deleteLaboratory } } = useLaboratoryApi();
    ...
    const { mutateAsync: deleteMutation } = useMutation([deleteLaboratoryKey], deleteLaboratory); // Use a mutation to remove an entry.
