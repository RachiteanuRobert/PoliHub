 ...
 */
export const useSubjectTableController = (search: string) => {
    const { getSubjects: { key: queryKey, query }, deleteSubject: { key: deleteSubjectKey, mutation: deleteSubject } } = useSubjectApi();
    ...
    const { mutateAsync: deleteMutation } = useMutation([deleteSubjectKey], deleteSubject); // Use a mutation to remove an entry.
